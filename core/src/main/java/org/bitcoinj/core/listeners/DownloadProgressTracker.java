...
public class DownloadProgressTracker extends AbstractPeerDataEventListener {
    ...
    public void onChainDownloadStarted(Peer peer, int blocksLeft) {
        ...
        if (originalBlocksLeft == -1)
            ...
            System.out.println("Chain download switched to "+ peer);
        }
        if (blocksLeft == 0) {
            doneDownload();
    ...
    protected void progress(double pct, int blocksSoFar, Date date) {
        ...
        System.out.println(String.format(Locale.US, "Chain download %d%% done with %d blocks to go, block date %s", (int) pct, blocksSoFar,
                Utils.dateTimeFormat(date)));
    }
